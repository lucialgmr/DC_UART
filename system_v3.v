...
module SYSTEM (
    ...
    output [7:0] salida   //changed_2710 salida GPO
);
...
// Memory write strobe
assign ramcs = (ca[31:13]==19'b0000000000000000000); // 8K RAM
assign ra    = ca[12:2];

assign wrlanes = ramcs ? mwe : 4'b0000;
...
always @* begin                                   //changed //"Selección combinacional del origen de iodo"
  casex (ca[7:2])                                 //changed //"Se usan ca[7:2] para discriminar registros de 32 bits"
    ...
    // --- Nueva UARTB0 -------------------------------------------------------
    6'b100000: iodo <= {24'hx, uartb0_do};       //changed //"RX_data UARTB0 en 0xE000_0080"
    6'b100001: iodo <= {27'hx, ove_b0, fe_b0,
                               tend_b0, thre_b0,
...
// Nueva UART con modo ráfaga (UARTB_CORE)
...
always @(posedge cclk or posedge reset) begin        //changed
    if (reset) begin                                 //changed
        irqvect[0] <= 30'h0;                         //changed
        irqvect[1] <= 30'h0;                         //changed
        irqvect[2] <= 30'h0;                         //changed
        irqvect[3] <= 30'h0;                         //changed
        irqvect[4] <= 30'h0;                         //changed
    end else begin                                   //changed
        // Vectores 0..3 mapeados en 0xE00000F0-0xE00000FC      //changed
        if (irqcs & ca[4] & (mwe==4'b1111)) begin    //changed
            irqvect[ca[3:2]] <= cdo[31:2];           //changed //"Dirección escrita según ca[3:2]"
        end                                          //changed
        // Vector 4 mapeado en 0xE00000E4                         //changed
        if (irqcs & (~ca[4]) & (ca[3:2]==2'b01) &    //changed
            (mwe==4'b1111)) begin                    //changed
            irqvect[4] <= cdo[31:2];                 //changed
        end                                          //changed
    end                                              //changed
end                                                  //changed
